...
- терминальный опыт: чёрная визуальная тема, скорость;
- работа с агентами и сессиями;
- project/session/workspace-подход;
...
## 16.0. Что уже сделано

Workspace — это git worktree. Список worktrees читается из
`git worktree list` при каждом обновлении git-статуса и нигде не хранится, поэтому
worktrees, созданные вручную, через `claude --worktree` или `codex --worktree`,
видны наравне с созданными в Relay.

- **Создание.** «Новый worktree…» в меню `+`, в меню Projects и в палитре:
  имя ветки, от чего ответвиться, что запустить (пресет) и необязательный первый
  промпт для агента. Папка — `~/.relay/worktrees/<репозиторий>/<ветка>`
  (`~/.relay-dev/…` у Relay Dev), вне репозитория, чтобы дерево файлов, поиск и
  TODO основной копии не заходили во вторую. Новая ветка создаётся с
  `--no-track`; существующая открывается как есть.
- **Сайдбар.** Пока worktree один, ничего не меняется. Когда их больше, сессии
  группируются по worktree: сначала папка проекта, затем остальные в порядке git.
  В заголовке — ветка, diff, общий статус свёрнутой группы и `+` для сессии
  именно в этом worktree.
- **Статус и комментарий.** У каждого worktree может быть статус работы —
  «К выполнению», «В работе», «На проверке», «Готово» — и комментарий. Статус
  показан заполняющимся кружком рядом с именем, комментарий — строкой под
  заголовком, целиком во всплывающей подсказке. Задаются из контекстного меню
  заголовка или агентом через `relay worktree set`; хранятся в файле рабочего
  пространства и исчезают вместе с worktree.
- **Принадлежность сессии** определяется по папке, в которой её запустили: самый
  глубокий worktree, который её содержит. Протокол демона не менялся.
- **Правая панель** (Git, файлы, поиск, TODO, определения, история) и новые сессии
  работают в worktree выбранной сессии или того, чей заголовок нажат.
- **Удаление** — из контекстного меню заголовка, с подтверждением. Перед
  вопросом Relay проверяет ветку (на заголовке крутится индикатор), и вопрос
  уже говорит, что с ней будет: удалится, останется — и сколько в ней
  невлитых коммитов, — или останется, потому что её создал не Relay. Если есть
  незакоммиченные изменения, кнопка так и называется: «Удалить вместе с
  изменениями»; если ветка останется — «Удалить, ветку оставить». Сессии в
  worktree закрываются. Ветка удаляется, только если её
  создал Relay и git доказывает, что её работа уже есть в основной ветке
  удалённого репозитория (`origin/HEAD`, иначе `main` или `master`), — влита
  слиянием, squash или rebase; иначе остаётся, а в уведомлении об этом есть
  кнопка «Удалить ветку». Она удаляет ветку, только пока та указывает на тот же
  коммит, что и в момент проверки.
- **Очистка.** «Убрать лишние worktrees…» — в контекстном меню заголовка, в
  меню Projects и в палитре, когда кроме папки проекта есть что убирать. Окно
  показывает каждый worktree, который можно удалить, и что этому мешает:
  блокировка, агент работает или ждёт ответа, git не смог его прочитать,
  коммиты detached `HEAD`, которых нет больше нигде, незакоммиченные файлы —
  их можно согласиться потерять, но только в том количестве, что есть сейчас.
  Неотправленные коммиты в ветке не мешают: ветка остаётся. Фильтры — «Влитые»,
  «Без изменений», «Без активности более N дней». Заранее отмечены влитые,
  чистые, без сессий и нетронутые хотя бы сутки. Удаляются по одному, каждый
  перечитывается прямо перед удалением; в конце — одно уведомление: сколько
  удалено, какие ветки оставлены и что git отказался делать, его словами.
- **Команда `relay`** в каждом терминале Relay — для агента не меньше, чем для
  человека: `relay worktree list`, `current`, `create`, `rm`, `set`. Её
  выполняет само приложение тем же кодом, что и окна, поэтому результат сразу
  виден в сайдбаре; `--json` даёт ответ для программы, `relay help` — всё
  остальное. Работает, пока запущен Relay.
- **Переключение веток.** Ветка, открытая в другом worktree, помечена в окне
  веток, и выбор ведёт в тот worktree.

Сервисы, Docker и порты пока работают в папке проекта — см. `docs/ROADMAP.md`.
...
Правка файлов из этого списка исключена: файл открывается в панели рядом с
терминалом, подсвечивается разбором tree-sitter и сохраняется по ⌘S, при уходе
фокуса и при закрытии панели. Причина в том, ради чего написано всё остальное:
агент в соседней панели читает файл с диска, и правка на одну строку не должна
означать переход в другое приложение. По ⌘-клику редактор переходит к месту, где
имя объявлено: тот же разбор, только запросом tags самой грамматики, плюс индекс
проекта, построенный однократным чтением файлов.

Границу это сдвигает ровно на одно деление. Языковых серверов, автодополнения,
рефакторингов и сборки здесь по-прежнему нет, а переход знает имена, а не типы:
когда одно имя объявлено в двух местах, редактор показывает оба, а не выбирает
за пользователя. Редактор знает про файл ровно столько, сколько нужно, чтобы его
прочитать, поправить, записать и найти, откуда взялось имя под курсором.
